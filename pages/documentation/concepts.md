...
### Model Objects
Models are the _things_ that exist within the story. They physically exist as web content and have properties that alter their appearance. The properties may be animated with scripts.

| Model | Description  |
|:-:|---|
| Canvas | The canvas is the drawing surface of the animation.  It is mapped to a single <code>div</code> element or the <code>body</code> element of an HTML page. The HTML element might be any rectangular size. In particular, a body tag stretches to fit the browser window. There is only one canvas. |
